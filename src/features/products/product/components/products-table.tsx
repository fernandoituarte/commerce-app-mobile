...
export function ProductsTable({
  ...
  // Derive cardWidth from actual container width measured via onLayout.
  // On tablet, OrgShell renders a 264 px sidebar; using window width would
  // oversize cards by exactly that amount. onLayout gives the true content area.
  // Margin formula: each card has `margin: GAP` on all sides, so total
  // horizontal margin per row = NUM_COLUMNS * 2 * GAP (not (NUM_COLUMNS+1)*GAP).
  const cardWidth =
    tableWidth > 0
      ? (tableWidth - PADDING * 2 - NUM_COLUMNS * 2 * GAP) / NUM_COLUMNS
      : 0;
  ...
  return (
    ...
    >
      {tableWidth === 0 ? null : !isLoading && (!data || data.items.length === 0) ? (
        <EmptyState
        ...
        >
          <View style={[styles.imgWrap, { backgroundColor: theme.surface }]}>
            ...
            {!item.isActive && (
              ...
              >
                <Text style={styles.overlayLabel}>
                  {t("org.products.inactive")}
                </Text>
              </View>
            )}
            {item.stock === 0 && !item.deletedAt && (
              <View
                style={[
                  styles.overlay,
            ...
            <Pressable
              ...
              onPress={
                canWrite
                  ? () => (item.deletedAt !== null ? onRestore(item) : onDelete(item))
                  : undefined
              }
              style={[styles.cardAction, !canWrite && { opacity: 0.4 }]}
            >
              <Ionicons
                name={
                  item.deletedAt !== null ? "refresh-outline" : "trash-outline"
                }
                size={14}
                color={
                  item.deletedAt !== null ? theme.primary : theme.textMuted
                }
              />
            </Pressable>
          </View>
          <View style={styles.cardBody}>
            ...
            <View style={styles.cardRow}>
              ...
              <Badge
                label={`${item.stock}`}
                tone={item.stock < 10 ? "danger" : "neutral"}
              />
            </View>
            {item.category?.name ? (
              <Text
